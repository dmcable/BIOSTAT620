...
1. Start an RStudio Quarto project titled `p1`. Place it inside a folder named `BIOSTAT620_pset_sol`. Start a Quarto document called `beginning.qmd`. We recommend you edit in source mode rather than visual. Also copy this file `pset-01-unix-quarto.qmd` and move it to the `p1` directory. You will input some answers to this file.

2. (1 points) Create a directory called `img` and save a screen shot of your RStudio session for the project (titled img_2). Include your screenshot in the Quarto document. 

3. (1 point) Next, in your Quarto document, define variables $a=1, b=-1, c=-2$ and print out the solutions to $f(x) = ax^2+bx+c=0$. Solving the equation with the quadratic formula, do not report complex solutions, only real numbers. 

4. (1 point) Include a graph of $f(x)$ versus $x$ for $x \in (-5,5)$. 
...
11. Navigate to `BIOSTAT620_pset_sol` in Terminal using `cd`. Create a Github instance by using:
